...
PeerChat::PeerChat(QJsonObject *config, const char *peer)
{
	...
	connect(input,
	    &QLineEdit::returnPressed, this, &PeerChat::create_message);
	...
	tunnel = new Tunnel(this, config, peer_id, false);
}
 ...
 */
void
PeerChat::message_show(const char *msg, u_int64_t id, Qt::GlobalColor color)
{
	QVariant		val;
	qulonglong		qid;
	QModelIndex		index;
